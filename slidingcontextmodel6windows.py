...
class CascadeTransformer(nn.Module):
    ...
    def forward(self, x, tgt,  mask=None, mode="eval"):
        ...
        for i, node in enumerate(self.nodes):
            ...
            x, mask, latent_memory= node(x, tgt, prev_node_output=prev_node_output, src_mask=mask, is_final_node=is_final_node, latent_memory=latent_memory)
            prev_node_output = x
...
class ReasoningModelGUI:
    def __init__(self, root):
        ...
        self.learning_rate = tk.DoubleVar(value=0.00005)
        ...
        try:
            ...
            for epoch in range(self.epochs.get()):
                ...
                def build_training_tokens_batch(batch):
                    bos = self.tokenizer.bos_token_id or self.tokenizer.cls_token_id or 0
                    sep = self.tokenizer.eos_token_id or self.tokenizer.sep_token_id or 1
                    eos = sep
                ...
                for i in range(0, len(dataset), self.batch_size.get()):

                    if self.stop_training.is_set():
                            logging.info("Training stopped by user.")
                            messagebox.showinfo("Info", "Training stopped by user.")
                            return
                    logging.debug(f'Epoch: {epoch + 1}, Batch: {i + 1}')
                    ...
                    batch_loss = 0
                    step_count = 0
                    batch = dataset[i:i + self.batch_size.get()]
                    if not batch:
                        continue
                    ...
                    print(f"Batch iteration {i}, Loss: {avg_loss}")
                    gc.collect()
                    torch.cuda.empty_cache()
